...
class WeTTYTestBridge {
  private wettyProcess: ChildProcess | null = null;
  private browser: Browser | null = null;
  private expressServer: express.Application | null = null;
  private httpServer: ReturnType<typeof createServer> | null = null;
  private io: SocketIOServer | null = null;
  private wettyPort = 3000;
  private bridgePort = 3100;
  ...
  private async ensureWeTTYRunning(): Promise<void> {
  ...
  private async initAllTabs(): Promise<void> {
  ...
  private async initTab(tabId: number): Promise<void> {
    ...
    await page.waitForFunction(() => {
      return (window as any).wetty_term !== undefined;
    }, { timeout: 15000 }).catch(() => {
      console.log(`Tab ${tabId}: 等待 wetty_term 初始化超时`);
    });

    const status = await page.evaluate(() => ({
      hasTerm: !!(window as any).wetty_term,
      hasBuffer: !!(window as any).wetty_term?.buffer?.active
    }));
  ...
  private startContentPolling(tabId: number): void {
    const interval = setInterval(async () => {
      ...
      try {
        const terminalData = await tab.page.evaluate(() => {
          const term = (window as any).wetty_term;
          ...
          try {
            ...
            return { lines: ['读取终端错误: ' + (e as Error).message], cursorX: 0, cursorY: 0, cols: 80, rows: 24, error: String(e) };
          }
        });
  ...
  private async startExpressServer(): Promise<void> {
    ...
    this.io.on('connection', async (socket) => {
      ...
      socket.on('join-tab', (tabId: number) => {
      ...
      socket.on('leave-tab', (tabId: number) => {
        socket.leave(`tab-${tabId}`);
        console.log(`客户端 ${socket.id} 离开 Tab ${tabId}`);
      });

      socket.on('send-input', async (data: { tabId: number; text: string }) => {
      ...
      socket.on('send-key', async (data: { tabId: number; key: string }) => {
        const tab = this.tabs.get(data.tabId);
      ...
      socket.on('send-ctrl', async (data: { tabId: number; char: string }) => {
        const tab = this.tabs.get(data.tabId);
